    ...
    public class AudioSessionMultiSelector : IAudioMultiSelector, INotifyPropertyChanged
    {
        ...
        public bool LockSelection
        {
            ...
            set
            {
                ...
                // When locking, store the current session's process identifier for auto-selection
                if (value && CurrentSession != null)
                {
                    LockedProcessIdentifier = CurrentSession.ProcessIdentifier;
                    if (FLog.FilterEventType(EventType.DEBUG))
                        FLog.Debug($"[{nameof(AudioSessionMultiSelector)}] Locked to process: {LockedProcessIdentifier}");
                }
                // When unlocking, optionally clear the locked process identifier
                else if (!value && AutoSelectLockedProcess)
                {
                    LockedProcessIdentifier = null;
                    if (FLog.FilterEventType(EventType.DEBUG))
                        FLog.Debug($"[{nameof(AudioSessionMultiSelector)}] Unlocked, cleared locked process identifier.");
                }
            }
        }
        ...
        /// <summary>
        /// Gets or sets whether to automatically select and focus on a process when it's locked.
        /// When enabled, locking a session will remember its process identifier and automatically select it when available.
        /// </summary>
        public bool AutoSelectLockedProcess
        {
            get => _autoSelectLockedProcess;
            set
            {
                _autoSelectLockedProcess = value;
                NotifyPropertyChanged();
            }
        }
        private bool _autoSelectLockedProcess = true;
        /// <summary>
        /// Gets or sets the process identifier of the locked session.
        /// When a session with this identifier is available, it will be automatically selected if <see cref="AutoSelectLockedProcess"/> is enabled.
        /// </summary>
        public string? LockedProcessIdentifier
        {
            get => _lockedProcessIdentifier;
            set
            {
                _lockedProcessIdentifier = value;
                NotifyPropertyChanged();
            }
        }
        private string? _lockedProcessIdentifier;
        ...
        private void AddSession(AudioSession session)
        {
            ...
            // Auto-select if this session matches the locked process identifier
            if (AutoSelectLockedProcess && !string.IsNullOrEmpty(LockedProcessIdentifier))
            {
                if (session.ProcessIdentifier.Equals(LockedProcessIdentifier, StringComparison.OrdinalIgnoreCase))
                {
                    // Temporarily unlock to allow setting the current session
                    var wasLocked = LockCurrentIndex;
                    LockCurrentIndex = false;

                    CurrentSession = session;

                    if (FLog.FilterEventType(EventType.INFO))
                        FLog.Info($"[{nameof(AudioSessionMultiSelector)}] Auto-selected locked process: {session.ProcessIdentifier} ({session.Name})");

                    // Restore lock state
                    LockCurrentIndex = wasLocked;
                }
            }
        }
        ...
        #region SelectByProcessIdentifier
        /// <summary>
        /// Attempts to find and select a session by its process identifier (e.g., "Spotify", "chrome", etc.).
        /// </summary>
        /// <param name="processIdentifier">The process identifier to search for (case-insensitive).</param>
        /// <param name="lockToProcess">If true, also enables LockSelection and stores this as the LockedProcessIdentifier.</param>
        /// <returns><see langword="true"/> if a matching session was found and selected; otherwise <see langword="false"/>.</returns>
        public bool SelectByProcessIdentifier(string processIdentifier, bool lockToProcess = false)
        {
            if (string.IsNullOrWhiteSpace(processIdentifier))
                return false;

            var matchingSession = Sessions.FirstOrDefault(s => 
                s.ProcessIdentifier.Equals(processIdentifier, StringComparison.OrdinalIgnoreCase));
            ...
            if (matchingSession != null)
            {
                ...
                if (FLog.FilterEventType(EventType.INFO))
                    FLog.Info($"[{nameof(AudioSessionMultiSelector)}] Selected session by process identifier: {processIdentifier} ({matchingSession.Name})");
            ...
            {
                if (FLog.FilterEventType(EventType.DEBUG))
                    FLog.Debug($"[{nameof(AudioSessionMultiSelector)}] Could not find session with process identifier: {processIdentifier}");

                return false;
            }
        }
        #endregion SelectByProcessIdentifier
