...
namespace VolumeControl.HotkeyActions
{
    ...
    public sealed class AudioSessionActions
    {
        ...
        [HotkeyAction(Description = "Locks to a specific process by name (e.g., 'Spotify', 'chrome'). The process will be automatically selected when available.")]
        [HotkeyActionSetting("Process Name", typeof(string), Description = "The name of the process to lock to (case-insensitive). Example: Spotify, chrome, firefox")]
        public void LockToProcess(object? sender, HotkeyPressedEventArgs e)
        {
            var processName = e.GetValue<string>("Process Name");
            
            if (!string.IsNullOrWhiteSpace(processName))
            {
                // Try to find and select the process
                if (MultiSelector.SelectByProcessIdentifier(processName, lockToProcess: true))
                {
                    VCAPI.ShowSessionListNotification();
                }
                else
                {
                    // Process not found, but still set it as the locked identifier so it will be auto-selected when it appears
                    MultiSelector.LockedProcessIdentifier = processName;
                    MultiSelector.LockSelection = true;
                    VCAPI.ShowSessionListNotification();
                }
            }
